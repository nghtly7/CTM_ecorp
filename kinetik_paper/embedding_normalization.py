...
def embedding(input1, input2="ecorp.npy"):
    """
    Embedding B: per-block normalization.
    Splits LL2's 32x32 into 16 blocks (8x8) and scales each block by its local sigma.
    ...
    for by in range(4):
        for bx in range(4):
            y0, x0 = by * 8, bx * 8
            block = LL2[y0:y0+8, x0:x0+8]
            sigma_block = float(block.std()) + 1e-12
            bits_block = bits[y0:y0+8, x0:x0+8].astype(np.float32)
            bipolar = (bits_block * 2 - 1)
            pattern = bipolar * sigma_block
            LL2_w[y0:y0+8, x0:x0+8] = LL2_w[y0:y0+8, x0:x0+8] + alpha * pattern
