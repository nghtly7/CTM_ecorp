...
def embedding(input1, input2):
	...
	# Select mid-frequency region indices: zigzag rank 10..30
	...
	# Build list of all modifiable coefficient positions across subbands
	...
	# ECC encode (Hamming 15,11) with padding
	...
	# PRNG seed from original image basename for reproducibility (must match detection)
	...
	# Precompute per-subband variance map for masking
	var_maps = []
	for (_, sb_arr) in subbands:
		var_maps.append(_block_variance_map(sb_arr))

	# Modify DCT coefficients block-wise per subband
	# We'll build DCT arrays, modify, then inverse
	modified_subbands = []
	embed_positions = []  # ordered list of used positions (si, by, bx, u, v)
	...
	for bi in range(len(payload_perm)):
		...
		for ri in range(R):
			...
			# local masking scale from variance map
			vm = var_maps[si]
			# Clip by, bx in case of shape edge
			...
			# DCT array is contiguous; compute top-left of this block
	...
	# Optional WPSNR-based precise adjustment to reach target WPSNR (with one fine-tuning pass)
	if compute_wpsnr is not None:
		try:
			target_wpsnr = 64.0
			wpsnr_val = float(compute_wpsnr((I_f32 * 255.0).astype(np.uint8), (I_wm * 255.0).astype(np.uint8)))
			if wpsnr_val < target_wpsnr:
				# Scale factor to reach target: new = old + 20*log10(1/scale) => scale = 10^((old-target)/20)
				scale = float(10 ** ((wpsnr_val - target_wpsnr) / 20.0))
				scale = float(np.clip(scale, 0.25, 1.0))
				I_wm = np.clip(I_f32 + (I_wm - I_f32) * scale, 0.0, 1.0)
				# Fine-tuning pass
				wpsnr_val2 = float(compute_wpsnr((I_f32 * 255.0).astype(np.uint8), (I_wm * 255.0).astype(np.uint8)))
				if (target_wpsnr - wpsnr_val2) > 0.2:
					scale2 = float(10 ** ((wpsnr_val2 - target_wpsnr) / 20.0))
					scale2 = float(np.clip(scale2, 0.5, 1.0))  # smaller correction
					I_wm = np.clip(I_f32 + (I_wm - I_f32) * scale2, 0.0, 1.0)
		except Exception:
			# If WPSNR computation fails (e.g., missing csf.csv), proceed without adjustment
			pass
	...
	state = {
		...
		"M_embed": M_embed,  # (N, 5) tuples: (subband_index, by, bx, u, v) in the mapping order
		"seed": int(seed),
		"ecc_params": ecc_params,
		"alpha_matrix": alpha_mat.astype(np.float32),
		"tau": float(0.88),  # placeholder threshold; to be tuned via ROC
	...
	# Save state next to the CTM_ecorp folder root if possible
	...
	# Save output BMP as "groupname imageName.bmp" in original image folder
	# orig_dir = os.path.dirname(orig_path)
	# orig_name = os.path.basename(orig_path)
	# name_wo_ext, _ = os.path.splitext(orig_name)
	# out_name = f"ecorp {name_wo_ext}.bmp"
	# out_path = os.path.join(orig_dir if orig_dir else os.getcwd(), out_name)
	# cv2.imwrite(out_path, I_wm_u8)

	# Return the watermarked image ndarray for compatibility with test.py
	return I_wm_u8
