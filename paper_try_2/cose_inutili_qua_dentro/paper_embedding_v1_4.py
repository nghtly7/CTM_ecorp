...
# === GLOBAL PARAMS (attenuati per privilegiare WPSNR) ===
global beta, gamma, Q, soft_t, soft_k, band_scale
beta   = 0.3      # sensibilità all'energia DCT del blocco
gamma  = 0.4      # solo attenuazione via attack-map (mai boost)
Q      = 0.6      # quantizzazione correttiva sui coeff marcati
soft_t = 0.30     # soglia "blocchi piatti" (skip totale)
soft_k = 1.0      # (non usato al momento)
# bande fini più forti, bande grossolane più deboli (ancora più "soft" rispetto a v1_3)
band_scale = [1.0, 1.0, 0.5, 0.5]


def embedding(input1, input2='ecorp.npy'):
    """
    Embed del watermark binario (1024 bit) su immagine 512x512 (grayscale).
    Modifiche vs v1_3:
      - Closed-loop su WPSNR (identico a v1_3) per centrare 58.02±0.02 dB
      - DC-lock nei 4x4 (ripristino C[0,0] dopo l'embed)
      - Mask mid-band ridotta a 3 coeff: [(1,1),(2,0),(0,2)]
      - band_scale indebolito: [1.0,1.0,0.5,0.5]
      - attack_map usata solo come attenuazione (1 - gamma*m), niente boost
      - clamp alpha_block più stretto [0.1, 1.5] * alpha_curr
      - Skip dei blocchi piatti mantenuto (opzione A)
    """
    # Parametri embedding
    ...
    # 3) set mid-band e PN (MASK = 3 COEFFICIENTI)
    mask = [(1,1), (2,0), (0,2)]
    rng = np.random.default_rng(FIXED_SEED)
    ...
    # 4) Attack-map (usata SOLO come attenuazione nel calcolo alpha_block)
    ...
    for b in range(4):
        ...
        for by in range(0, 64, 4):
            for bx in range(0, 64, 4):
                ...
                # clamp "più stretto" già nel dry-run
                alpha_block = float(np.clip(alpha_block, 0.1*alpha, 1.5*alpha))
    ...
    def do_embed_with_alpha(alpha_curr: float) -> np.ndarray:
        ...
        for b in range(4):
            ...
            for by in range(0, 64, 4):
                for bx in range(0, 64, 4):
                    ...
                    # alpha per-blocco (solo attenuazione da attack_map)
                    ...
                    # Skip totale blocchi piatti (Opzione A)
                    if E < soft_t * E_mean:
                        idx += 1
                        continue
                    ...
                    # clamp più stretto
                    alpha_block = float(np.clip(alpha_block, 0.1*alpha_curr, 1.5*alpha_curr))

                    # iniezione watermark (DC-LOCK: preserva C[0,0])
                    C00 = float(C[0,0])
                    for k,(u,v) in enumerate(mask):
                        C[u,v] += alpha_block * p_use[k]

                    # quantizzazione correttiva (leggera)
                    for (u,v) in mask:
        ...
        new_coeffs[1] = (LH3n, HL3n, HH3_0)
        new_coeffs[2] = (LH2n, HL2n, HH2_0)
        new_coeffs[3] = (LH1_0, HL1_0, HH1_0)
        Iw = pywt.waverec2(new_coeffs, wavelet='db2')
        Iw = np.clip(Iw, 0, 255).astype(np.uint8)
        return Iw

    # 7) REFINEMENT loop su WPSNR reale
    Iw_best = None
    for _ in range(MAX_ITERS):
        ...
        # closed-loop: ricalibra alpha usando l'MSE misurata
...
if __name__ == "__main__":
    ...
    ap.add_argument("input_img", type=str, help="path immagine 512x512 grayscale")
    ap.add_argument("--wm", type=str, default="ecorp.npy", help="path a file .npy con 1024 bit (0/1)")
    ap.add_argument("--out", type=str, default="out_wm.png", help="output image path")
    args = ap.parse_args()

    out = embedding(args.input_img, args.wm)
