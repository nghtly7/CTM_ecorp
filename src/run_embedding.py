...
def run_embedding():
    ...
    output_dir = f"watermarked_images/"
    ...
    for i, image_file in enumerate(image_files, 1):
        ...
        output_filename = f"{GROUP_NAME}_{image_file}"
        ...
        try:
            ...
            if original_image.shape != watermarked_image.shape:
                ...
                if watermarked_image.ndim == 3 and watermarked_image.shape[2] == 3:
                    ...
                    watermarked_gray = cv2.resize(
                        watermarked_image, (original_image.shape[1], original_image.shape[0]), interpolation=cv2.INTER_LINEAR)
            else:
                # shapes ok
            ...
            try:
                ...
                wpsnr_value = float(wpsnr(original_image.astype(np.float32)/255.0,
                                    watermarked_gray.astype(np.float32)/255.0))
