...
# choose from: light, medium, aggressive, coarse, fine
ATTACK_STRATEGY = "medium"

# -------------------------------------------------------------------------
# Attack presets: each profile defines the parameter grids for each attack
# -------------------------------------------------------------------------
ATTACK_PROFILES = {
    # fast, conservative tests: few coarse steps (safer, faster)
    "light": OrderedDict([
        ('jpeg', ('QF', [[q] for q in range(60, 39, -10)])),  # 60,50,40
        ('blur', ('sigma', [[round(s, 2), round(s, 2)] for s in np.arange(0.6, 1.6, 0.5)])),  # 0.6,1.1
        ('median', ('kernel_size', [[3, 3]])),
        ('awgn', ('std_seed', [[s, 123] for s in [5.0, 10.0]])),
        ('resize', ('scale', [[0.8], [0.6]])),
        ('sharp', ('sigma_alpha', [[0.5, 0.5]])),
    ]),

    # default medium: balanced coverage and speed (your previous "main" profile)
    "medium": OrderedDict([
    ...
    # aggressive: many steps, finer grid, more combinations (slower but stronger)
    "aggressive": OrderedDict([
        ('jpeg', ('QF', [[q] for q in range(95, 29, -5)])),  # 95..30
        ('blur', ('sigma', [[round(s, 2), round(s, 2)] for s in np.arange(0.2, 2.6, 0.2)])),
        ('median', ('kernel_size', [[3, 3], [5, 5], [7, 7]])),
        ('awgn', ('std_seed', [[s, 123] for s in [2.0, 5.0, 8.0, 12.0, 16.0, 20.0]])),
        ('resize', ('scale', [[s] for s in [0.95, 0.9, 0.8, 0.7, 0.6, 0.5]])),
        ('sharp', ('sigma_alpha', [[0.3, 0.5], [0.5, 0.5], [1.0, 1.0], [1.0, 1.5]])),
    ...
    # fine/coarse: helper profiles for progressive search (coarse then fine)
    "coarse": OrderedDict([
        ('jpeg', ('QF', [[q] for q in range(80, 29, -20)])),  # 80,60,40
        ('blur', ('sigma', [[round(s, 2), round(s, 2)] for s in np.arange(0.8, 2.0, 0.6)])),
        ('awgn', ('std_seed', [[s, 123] for s in [10.0, 20.0]])),
    ]),
    "fine": OrderedDict([
        ('jpeg', ('QF', [[q] for q in range(55, 40, -5)])),  # 55..41 step 1
        ('blur', ('sigma', [[round(s, 2), round(s, 2)] for s in np.arange(0.4, 1.6, 0.1)])),
        ('awgn', ('std_seed', [[s, 123] for s in np.arange(5.0, 21.0, 1.0)])),
    ]),
}
...
def _generate_attack_list(profile: str) -> List[Dict[str, Any]]:
    """Produce ordered single and paired attack combinations (least -> most aggressive)."""
    if profile not in ATTACK_PROFILES:
        raise ValueError(f"Unknown profile '{profile}'. Available: {list(ATTACK_PROFILES.keys())}")

    attack_params_db = ATTACK_PROFILES[profile]
...
def _run_attack_type_worker(
    ...
    # print(f"   [Worker {os.getpid()}] --- No successful attack found for type {attack_type_id}.")
    return (0.0, None)
...
def find_best_attack(
    ...
    attack_list = _generate_attack_list(profile=ATTACK_STRATEGY)
